...
def plot_likelihood_distribution(ll_value_obs, ll_values):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    (ll_bin_counts, ll_bin_edges, _) = ax.hist(ll_values, bins=100, histtype='stepfilled')
    ...
    ax.text(ll_value_obs, y_value * 1.2, s)
    fig.savefig('ll.png')
...
def main():
    ...
    print(f'Number of ll values required: {len(bootstrap_data_count)}')
    print(f'')
    
    if number_of_ll_values_loaded < len(bootstrap_data_count):
