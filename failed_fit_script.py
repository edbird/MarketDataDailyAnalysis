...
def plot_failed_fit(index, bin_midpoints, bin_contents, amplitude, mean, stddev):
    ...
    fig.savefig(f'plot_aapl_likelihood/failed_fit/failed_fit_{index}_2.png')
    plt.close() 
...
def main():
    ...
    # Load difference
    index = 12853
    eod_aapl_us_diff = numpy.loadtxt(f'plot_aapl_likelihood/bootstrap_data_txt/data_{index}.txt')
    ...
    mean = eod_aapl_us_diff.mean() #0.0
    stddev = 1.2 * aapl_us_diff_std
    
    x0 = [amplitude, mean, stddev]
    
    optimize_result = perform_fitting_procedure(eod_aapl_us_diff, x0, index, True)
    ...
    fig.savefig(f'plot_aapl_likelihood/failed_fit/fit_{index}.png')
